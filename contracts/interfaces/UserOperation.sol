...
struct UserOperation {
    address sender; // 链上account，最初链上没有部署
    uint256 nonce; // account的nonce，不是EOA的
    bytes initCode; // account的bytecode
    bytes callData; // account在链上调用的
    uint256 callGasLimit;
    uint256 verificationGasLimit;
    uint256 preVerificationGas;
    uint256 maxFeePerGas;
    uint256 maxPriorityFeePerGas;
    bytes paymasterAndData; // paymer的地址，以及参数
    bytes signature; // EOA的签名，当没有指定Aggregator的时候，signature是必要的，否则是可选的
}
